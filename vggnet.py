...
def inference_op(input_op,keep_prob):
	...
	fc6=fc_op(reshp1,name='fc6',n_out=4096,p=p)
...
def time_tensorflow_run(session,target,feed,info_string,num_batches):
	...
	if vr <0:
		vr=0
	sd=math.sqrt(vr)
	print('%s: %s across %d stpes, %.3f +/- %.3f sec / batch '%(datetime.now(),info_string,num_batches,mn,sd))
...
batch_size=32 
num_batches=10
run_benchmark(batch_size,num_batches)
